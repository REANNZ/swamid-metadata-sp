...
if (isset($_SERVER['eduPersonPrincipalName'])) {
  ...
  $errors .= 'Missing eduPersonPrincipalName/subject-id in SAML response ' .
    str_replace(array(
      'ERRORURL_CODE', 'ERRORURL_CTX'), array('IDENTIFICATION_FAILURE', 'eduPersonPrincipalName'
    ), $errorURL);
}
...
function showShadowEntities()
{
  ...
  $entityHandler = $config->getDb()->prepare(
    'SELECT id, `entityID`, `lastValidated`
    FROM Entities
    WHERE status = 6 AND id NOT IN (SELECT publishedId FROM Entities WHERE `status`=5 OR `status`=2);'
  );
  $entityHandler->execute();
  printf(
    '        <h5>Entities only in shadow</h5>%s' .
    '        <table id="shadow-table" class="table table-striped table-bordered">%s' .
    '          <thead><tr><th>Id</th><th>EntityID</th><th>Created</th></tr></thead>%s',
    "\n",
    "\n",
    "\n"
  );
  while ($entity = $entityHandler->fetch(PDO::FETCH_ASSOC)) {
    printf(
      '          <tr><td><a href=./?removeEntity=%d target="_blank"><i class="fas fa-trash"></i></a>%d</td>' .
      '<td>%s</td><td>%s</td></tr>%s',
      $entity['id'],
      $entity['id'],
      htmlspecialchars($entity['entityID']),
      $entity['lastValidated'],
      "\n"
    );
  }
...
function showSoftDeletedEntities()
{
  ...
  $entityHandler->execute();
  printf(
    '        <h5>Entities in Soft Delete</h5>%s' .
    '        <table id="softDel-table" class="table table-striped table-bordered">%s' .
    '          <thead><tr><th>Action</th><th>EntityID</th><th>Removed</th></tr></thead>%s',
    "\n",
    "\n",
    "\n"
  );
  while ($entity = $entityHandler->fetch(PDO::FETCH_ASSOC)) {
    printf(
      '          <tr><td><a href="./?showEntity=%d" target="_blank">' .
      ...
      '<button class="btn btn-outline-primary">Create Draft</button>' .
      '</form></td><td>%s</td><td>%s</td></tr>%s',
      $entity['id'],
      $entity['id'],
      htmlspecialchars($entity['entityID']),
      $entity['lastUpdated'],
...
function showPublishedPendingEntities()
{
  ...
  $entityHandler->execute();
  printf(
    '        <h5>Entities from Pending that is Published</h5>
  ...
  while ($entity = $entityHandler->fetch(PDO::FETCH_ASSOC)) {
    if ($lastId != $entity['id']) {
      printf(
        '          <tr><td><a href=./?showEntity=%d target="_blank">%d</a></td>' .
        '<td>%s</td><td>%s</td><td>%s</td></tr>%s',
        $entity['id'],
        $entity['id'],
        htmlspecialchars($entity['entityID']),
        $entity['lastUpdated'],
        htmlspecialchars($entity['email']),
        "\n"
      );
      $lastId = $entity['id'];
    }
...
function showUsers()
{
  ...
  if (isset($_GET['user'])) {
    ...
    if ($user = $userHandler->fetch(PDO::FETCH_ASSOC)) {
      ...
      while ($entity = $entitiesHandler->fetch(PDO::FETCH_ASSOC)) {
        ...
        printf(
          '          <tr><td>%s</td><td><a href=./?showEntity=%d target="_blank">%s<a></td></tr>%s',
          $statusName,
          $entity['id'],
          htmlspecialchars($entity['entityID']),
          "\n"
  ...
  $usersHandler = $config->getDb()->prepare(
    'SELECT `id`, `userID`, `email`, `fullName`, `lastSeen`, COUNT(entity_id) AS count
  ...
  printf(
    '        <h5>Users</h5>
        <table id="user-table" class="table table-striped table-bordered">
          <thead><tr><th>UserID</th><th>Email</th><th>Name</th><th>Last seen</th>' .
    '<th>Number of entities</th></tr></thead>%s',
    "\n"
  );
  while ($user = $usersHandler->fetch(PDO::FETCH_ASSOC)) {
    printf(
      '          <tr><td><a href="?action=users&user=%d">%s<a></td></td><td>%s</td>' .
      '<td>%s</td><td>%s</td><td>%d</td></tr>%s',
      $user['id'],
      htmlspecialchars($user['userID']),
...
function showValidationOutput()
{
  ...
  $entitiesHandler = $config->getDb()->prepare(
    'SELECT `id`, `entityID` , `validationOutput` FROM `Entities` WHERE `status` < 4 AND `validationOutput` != ""'
  );
  $entitiesHandler->execute();
  ...
  while ($entity = $entitiesHandler->fetch(PDO::FETCH_ASSOC)) {
    printf(
      '          <tr><td><a href=./?showEntity=%d target="_blank">%s</a></td><td>%s</td></tr>%s',
      $entity['id'],
      htmlspecialchars($entity['entityID']),
...
function showOrganizationDifference()
{
  ...
  $organizationHandler = $config->getDb()->prepare(
    ...
    ORDER BY `OrganizationName` COLLATE utf8mb4_swedish_ci, `OrganizationDisplayName` COLLATE utf8mb4_swedish_ci,' .
    ' `OrganizationURL` COLLATE utf8mb4_swedish_ci;"
  );
  ...
  foreach ($languages as $lang) {
    ...
    while ($organization = $organizationHandler->fetch(PDO::FETCH_ASSOC)) {
      if (
        ...
        $orgString = $organization['OrganizationName'] . $organization['OrganizationDisplayName'] .
          $organization['OrganizationURL'];
        $newOrgInfo = true;
        $orgsSame = false;
        while ($newOrgInfo && !$orgsSame && $orgInfo) {
          $orgInfoString = $orgInfo['OrganizationName'] . $orgInfo['OrganizationDisplayName'] .
            $orgInfo['OrganizationURL'];
...
function printNewOrgInfo($organization)
{
  printf(
    ...
    htmlspecialchars($organization['OrganizationDisplayName']),
    htmlspecialchars($organization['OrganizationURL']),
    $organization['OrganizationInfo_id'],
    $organization['OrganizationInfo_id'],
    $organization['OrganizationInfo_id'],
    "\n"
  );
...
function showMenu()
{
  ...
  print "\n    ";
  printf(
    '<a href="?action=softDel"><button type="button" class="btn btn%s-primary">SoftDeleted</button></a>',
    $menuActive == 'softDel' ? '' : HTML_OUTLINE
  );
  printf(
    '<a href="?action=pubPend"><button type="button" class="btn btn%s-primary">Published</button></a>',
    $menuActive == 'pubPend' ? '' : HTML_OUTLINE
  );
  printf(
    '<a href="?action=shadow"><button type="button" class="btn btn%s-primary">Shadow</button></a>',
    $menuActive == 'shadow' ? '' : HTML_OUTLINE
  );
  printf(
    '<a href="?action=users"><button type="button" class="btn btn%s-primary">Users</button></a>',
    $menuActive == 'users' ? '' : HTML_OUTLINE
  );
  printf(
    '<a href="?action=validation"><button type="button" class="btn btn%s-primary">Validation</button></a>',
    $menuActive == 'validation' ? '' : HTML_OUTLINE
  );
  printf(
    '<a href="?action=organizations"><button type="button" class="btn btn%s-primary">Organizations</button></a>',
    $menuActive == 'organizations' ? '' : HTML_OUTLINE
  );
  print "\n    <br>\n    <br>\n";
}
