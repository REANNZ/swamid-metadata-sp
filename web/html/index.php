...
function showEntityList($show)
{
  ...
  switch ($show) {
    ...
    case 'SP':
      ...
      showMenu('SPs', $query);
      $extraTH = '<th>Anon</th><th>Pseuso</th><th>Pers</th><th>CoCo v1</th>' .
        '<th>CoCo v2</th><th>R&S</th><th>ESI</th><th>SIRTFI</th><th>SIRTFI2</th>';
      break;
    case 'All':
  ...
  printf(
    '    <div class="table-responsive"><table id="entities-table" class="table table-striped table-bordered">
      <thead>
        <tr>
          <th>entityID</th>
          <th>Published in</th>
...
function showMenu($menuActive, $query = '')
{
  ...
  printf(
    '<a href="./?show=All%s"><button type="button" class="btn btn%s-primary">All in %s</button></a>',
    $query,
    $menuActive == 'all' ? '' : HTML_OUTLINE,
    $federation['displayName']
  );
  printf(
    '<a href="./?show=IdP%s"><button type="button" class="btn btn%s-primary">IdP in %s</button></a>',
    $query,
    $menuActive == 'IdPs' ? '' : HTML_OUTLINE,
    $federation['displayName']
  );
  printf(
    '<a href="./?show=SP%s"><button type="button" class="btn btn%s-primary">SP in %s</button></a>',
    $query,
    $menuActive == 'SPs' ? '' : HTML_OUTLINE,
    $federation['displayName']
  );
  printf(
    '<a href="./?show=InterIdP"><button type="button" class="btn btn%s-primary">IdP via interfederation</button></a>',
    $menuActive == 'fedIdPs' ? '' : HTML_OUTLINE
  );
  printf(
    '<a href="./?show=InterSP"><button type="button" class="btn btn%s-primary">SP via interfederation</button></a>',
    $menuActive == 'fedSPs' ? '' : HTML_OUTLINE
  );
  printf(
    '<a href="./?show=Info%s"><button type="button" class="btn btn%s-primary">Info</button></a>',
    $query,
    $menuActive == 'info' ? '' : HTML_OUTLINE
  );
  print "\n";
}
...
function showEntity($entity_id, $urn = false)
{
  ...
  if ($entity = $entityHandler->fetch(PDO::FETCH_ASSOC)) {
    ...
    $display->showStatusbar($entities_id);
    printf(
      '    <div class="row">
      <div class="col">
        %s
    ...
    if ($entity['isIdP']) {
      $display->showIdP($entities_id, $oldEntity_id);
    }
    if ($entity['isSP']) {
      $display->showSp($entities_id, $oldEntity_id);
    }
    if ($entity['isAA']) {
      $display->showAA($entities_id, $oldEntity_id);
    }
...
function showList($entities, $show)
{
  ...
  printf(
    ...
    <table class="table table-striped table-bordered">
      ...
      <tr><th id="">eduGAIN-Export</th><td>%d</td></tr>',
    $federation['displayName'],
    $countSWAMID,
    $counteduGAIN
  );
  if ($show == 'All' || $show == 'SP') {
    printf(
      ...
      <tr>
        ...
        <th id="ecAnonymous">Anonymous</th><td>%d</td>
      </tr>
      <tr><th id="ecPseudonymous">Pseudonymous</th><td>%d</td></tr>
      <tr><th id="ecPersonalized">Personalized</th><td>%d</td></tr>
      <tr><th id="ecCocov1">CoCo v1</th><td>%d</td></tr>
      <tr><th id="ecCocov2">CoCo v2</th><td>%d</td></tr>
      <tr><th id="ecRandS">R&S</th><td>%d</td></tr>
      <tr><th id="ecesi">ESI</th><td>%d</td></tr>
      <tr><th id="ecHideFromDisco">DS-hide </th><td>%d</td></tr>',
      $countECanon,
      $countECpseuso,
      $countECpers,
      $countECcocov1,
...
function showInfo()
{
  ...
  printf(
    '    <div class="row">
      <div class="col">
        <br>
        <h3>%s Metadata Tool</h3>
        <p>Welcome to the %s Metadata Tool. With this tool you can browse and examine
          metadata available through %s.
        <h4>Public available information</h4>
        <p>To view entities, i.e. Identity Providers and Service Providers, available in %s, select a tab:<ul>
          <li><b>All in %s</b> lists all entities registered in %s.</li>
          <li><b>IdP in %s</b> lists Identity Providers registered in %s including identity assurance profiles.</li>
          <li><b>SP in %s</b> lists Service Providers registered in %s including requested entity categories.</li>
          <li><b>IdP via interfederation</b> lists Identity Providers imported into %s from interfederations.</li>
          <li><b>SP via interfederation</b> lists Service Providers imported into %s from interfederations.</li>
        </ul></p>
        ...
        <p>Login using the orange button at the top right corner of this page to add, update or request removal of
          your entites in %s. %s Operations authenticates and validates all updates before changes are
          published in the %s metadata. After login, help on adding/updating entites is available in the menu
        ...
        <p>If you do not have an active user account at a %s Identity Provider,
          %s.</p>',
    $federation['displayName'],
    $federation['displayName'],
    $federation['displayName'],
    ...
    $federation['displayName'],
    $federation['displayName'],
    $federation['noAccountHtml']
  );
  if ($federation['infoPageHtml']) {
    printf('
        <p>%s</p>', $federation['infoPageHtml']);
  }
  print "
      </div>
    </div>\n";
...
function showRemoveQueue()
{
  ...
  header(TEXT_PLAIN);
  $entities = $config->getDb()->prepare(
    'SELECT `id`, `entityID` FROM Entities WHERE `removalRequestedBy` > 0 AND `status` = 1'
  );
  $entities->execute();
  while ($row = $entities->fetch(PDO::FETCH_ASSOC)) {
    printf('%d %s%s', $row['id'], $row['entityID'], "\n");
...
function showInterfederation($type)
{
  ...
  if ($type == 'IDP') {
    ...
    $entityList = $config->getDb()->query(
      'SELECT `entityID`, `organization`, `contacts`, `scopes`, `ecs`, `assurancec`, `ra`
    ...
    foreach ($entityList as $entity) {
      ...
      printf(
        ...
        $entity['organization'],
        $entity['contacts'],
        htmlspecialchars($entity['scopes']),
        htmlspecialchars($entity['ecs']),
        htmlspecialchars($entity['assurancec']),
        htmlspecialchars($entity['ra']),
        "\n"
      );
    }
  } else {
    ...
    foreach ($entityList as $entity) {
      ...
      printf(
        ...
        $entityId_html,
        htmlspecialchars($entity['displayName']),
        htmlspecialchars($entity['serviceName']),
        $entity['organization'],
        $entity['contacts'],
        htmlspecialchars($entity['ec']),
        htmlspecialchars($entity['assurancec']),
        htmlspecialchars($entity['ra']),
        "\n"
      );
...
function showEntityFromMDQ($entityID)
{
  ...
  curl_setopt(
    $ch,
    CURLOPT_PROTOCOLS,
    $config->getFederation()['urlCheckPlainHTTPEnabled'] ? CURLPROTO_HTTP | CURLPROTO_HTTPS : CURLPROTO_HTTPS
  );
  ...
  if (
    $parsed_url &&
    in_array($url_scheme, $allowed_schemes) &&
