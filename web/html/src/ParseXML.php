...
class ParseXML extends Common
{
  ...
  protected const BIND_COMPANY = ':Company';
  protected const BIND_DEFAULT = ':Default';
  protected const BIND_EXTENSIONS = ':Extensions';
  protected const BIND_GIVENNAME = ':GivenName';
  protected const BIND_SUBCONTACTTYPE = ':SubcontactType';
  protected const BIND_SURNAME = ':SurName';
  protected const BIND_TELEPHONENUMBER = ':TelephoneNumber';
  ...
  protected const SUBJECT_ID_REQ_VALUES = array('subject-id', 'pairwise-id', 'none', 'any');
  ...
  public function parseXML()
  {
    ...
    $this->config->getDb()->prepare(
      'DELETE FROM `AttributeConsumingService_RequestedAttribute`
      WHERE `entity_id` = :Id'
    )->execute(array(self::BIND_ID => $this->dbIdNr));
    ...
    $this->config->getDb()->prepare(
      'UPDATE `Entities` SET `isIdP` = 0, `isSP` = 0, `isAA` = 0 WHERE `id` = :Id'
    )->execute(array(self::BIND_ID => $this->dbIdNr));
    $this->isIdP = false;
    $this->isSP = false;
    $this->isAA = false;
      ...
      switch ($child->nodeName) {
        ...
        default:
          $this->result .= $child->nodeType == 8
            ? ''
            : sprintf("Unknown element %s found in metadata.\n", $child->nodeName);
      }
  ...
  protected function parseExtensionsEntityAttributesAttribute($data)
  {
    $entityAttributesHandler = $this->config->getDb()->prepare(
      'INSERT INTO `EntityAttributes` (`entity_id`, `type`, `attribute`)
    ...
    if (! $data->hasAttribute('NameFormat') && $data->hasAttribute('Name')) {
      switch ($data->getAttribute('Name')) {
        ...
        default:
          $this->result .= sprintf(
            "Unknown Name (%s) in Extensions/EntityAttributes/Attribute.\n",
            htmlspecialchars($data->getAttribute('Name'))
          );
          break;
      }
    ...
    if ($data->getAttribute('NameFormat') == self::SAMLNF_URI) {
        ...
        if ($child->nodeName == self::SAML_SAMLA_ATTRIBUTEVALUE) {
          ...
          if ($attributeType == 'subject-id:req') {
            ...
            if (!in_array($attributeValue, self::SUBJECT_ID_REQ_VALUES)) {
              $this->error .= sprintf(
                "subject-id:req value MUST be exactly one of: %s; found: \"%s\"\n",
          ...
          $this->result .= sprintf(
            "Unknown Element Extensions -> EntityAttributes -> Attribute -> %s.\n",
            $child->nodeName
          );
        }
        $child = $child->nextSibling;
      ...
      $this->result .= sprintf(
        "Unknown NameFormat (%s) in Extensions/EntityAttributes/Attribute.\n",
  ...
  protected function parseIDPSSODescriptor($data)
  {
    ...
    if ($shibboleth10found && ! $saml1found) {
      ...
      $this->error .= 'IDPSSODescriptor claims support for urn:mace:shibboleth:1.0.' .
        " This depends on SAML1, no support for SAML1 claimed.\n";
    }
  }
      ...
      switch ($child->nodeName) {
        case self::SAML_SHIBMD_SCOPE:
          switch (strtolower($child->getAttribute('regexp'))) {
            ...
            default:
              $this->result .= sprintf(
                "IDPSSODescriptor->Extensions->Scope unknown value for regexp %s.\n",
        ...
        default:
          $this->result .= sprintf(
            "Unknown element IDPSSODescriptor->Extensions->%s found in metadata.\n",
            $child->nodeName
          );
      }
      $child = $child->nextSibling;
  ...
  protected function parseSPSSODescriptor($data)
  {
    ...
    if ($shibboleth10found) {
      $this->errorNB .= 'Protocol urn:mace:shibboleth:1.0 should only be used on IdP:s protocolSupportEnumeration,' .
        " found in SPSSODescriptor.\n";
    }
    ...
    while ($child) {
      switch ($child->nodeName) {
        case self::SAML_MD_SERVICENAME:
          $lang = $child->getAttribute('xml:lang') #NOSONAR $lnag is used in Bind above
          ...
          if ($child->getAttribute('Name')) {
            ...
            if (
              ...
              $this->warning .= sprintf(
                ...
                'in RequestedAttribute for index',
                $index,
                htmlspecialchars($friendlyName),
                $friendlyNames[$name]['desc']
              );
            }
            if ($child->getAttribute('NameFormat')) {
              ...
              switch ($nameFormat) {
                ...
                case 'urn:mace:shibboleth:1.0:attributeNamespace:uri':
                  $this->warning .=
                    sprintf(
                      "SAML1 NameFormat %s for %s in RequestedAttribute for index %d is not recomended.\n",
                      htmlspecialchars($nameFormat),
                      htmlspecialchars($name),
                      $index
                    );
                  break;
                default:
                  $this->warning .=
                    sprintf(
                      "NameFormat %s for %s in RequestedAttribute for index %d is not recomended.\n",
                      htmlspecialchars($nameFormat),
                      htmlspecialchars($name),
                      $index
                    );
              ...
              $this->warning .=
                sprintf(
                  "NameFormat is missing for %s in RequestedAttribute for index %d. %s\n",
                  htmlspecialchars($name),
                  $index,
                  'This might create problmes with some IdP:s'
                );
            }
            ...
            $this->error .=
              sprintf(
                "%sSPSSODescriptor->AttributeConsumingService[index=%d]->RequestedAttribute.\n",
                'A Name attribute is Required in ',
                $index
              );
          }
          break;
        default:
          $this->result .= $child->nodeType == 8
            ? ''
            : sprintf(
              "Unknown element SPSSODescriptor->AttributeConsumingService->%s found in metadata.\n",
              $child->nodeName
            );
      }
      $child = $child->nextSibling;
  ...
  protected function parseOrganization($data)
  {
    ...
    $organizationHandler = $this->config->getDb()->prepare(
      'INSERT INTO `Organization` (`entity_id`, `lang`, `element`, `data`)
  ...
  protected function parseContactPerson($data)
  {
    ...
    switch ($data->getAttribute('contactType')) {
      ...
      case 'other':
        if ($data->getAttribute(self::SAML_ATTRIBUTE_REMD)) {
          if (
            ...
            $this->result .= sprintf(
              "ContactPerson->Unknown subcontactType->%s.\n",
              $data->getAttribute(self::SAML_ATTRIBUTE_REMD)
            );
          }
        } else {
      ...
      default:
        $contactType = 'Unknown';
        $this->result .= sprintf(
          "Unknown contactType in ContactPerson->%s.\n",
    ...
    while ($child) {
      ...
      if ($value == '') {
        $this->error .= sprintf(
          "Error in uploaded XML. Element %s in contact type=%s is empty!\n",
          $child->nodeName,
          htmlspecialchars($data->getAttribute('contactType'))
        );
      }
  ...
  protected function parseSSODescriptorExtensionsDiscoveryResponse($data)
  {
    ...
    if ($data->hasAttribute('Binding') && $data->hasAttribute('Location') && $data->hasAttribute('index')) {
      if ($data->getAttribute('Binding') != 'urn:oasis:names:tc:SAML:profiles:SSO:idp-discovery-protocol') {
        $this->error .= sprintf(
          "Binding in %s with index = %d is NOT urn:oasis:names:tc:SAML:profiles:SSO:idp-discovery-protocol.\n",
          $data->nodeName,
          $data->getAttribute('index')
        );
      }
  ...
  protected function parseSSODescriptorExtensionsUIInfo($data, $type)
  {
    ...
    $urlHandler = $this->config->getDb()->prepare(
      'SELECT `nosize`, `height`, `width`, `status` FROM `URLs` WHERE `URL` = :URL;'
    );
          ...
          case self::SAML_MDUI_LOGO:
            ...
            if ($urlInfo = $urlHandler->fetch(PDO::FETCH_ASSOC)) {
              ...
              if (
                $urlInfo['width'] != $width &&
                $urlInfo['status'] == 0 &&
                $urlInfo['nosize'] == 0 && $urlInfo['width'] > 0
              ) {
                $this->error .= sprintf(
  ...
  protected function parseProtocolSupportEnumeration($data)
  {
    ...
    foreach (explode(' ', $protocolSupportEnumeration) as $protocol) {
      switch ($protocol) {
        ...
        default:
          $this->result .= sprintf(
            "Unknown protocol %s found in protocolSupportEnumeration for %s.\n",
      ...
      switch ($child->nodeName) {
        ...
        default:
          $this->result .= $child->nodeType == 8
            ? ''
            : sprintf(
              "Unknown element %sDescriptor->KeyDescriptor->KeyInfo->%s found in metadata.\n",
              $type,
              $child->nodeName
            );
      }
      ...
      switch ($child->nodeName) {
        ...
        default:
          $this->result .= $child->nodeType == 8
            ? ''
            : sprintf(
              "Unknown element %sDescriptor->KeyDescriptor->KeyInfo->X509Data->%s found in metadata.\n",
              $type,
              $child->nodeName
            );
      }
  ...
  protected function validateDigestMethod($data)
  {
    ...
    if (isset(self::DIGEST_METHODS[$algorithm])) {
      switch (self::DIGEST_METHODS[$algorithm]) {
        ...
        default:
          $this->result .= sprintf(
            "CommonTrait.php digestMethod[%s] have unknown status (%s).\n",
            htmlspecialchars($algorithm),
            self::DIGEST_METHODS[$algorithm]
          );
      }
    } else {
  ...
  protected function validateSigningMethod($data)
  {
    ...
    if (isset(self::SIGNING_METHODS[$algorithm])) {
      switch (self::SIGNING_METHODS[$algorithm]) {
        ...
        default:
          $this->result .= sprintf(
            "CommonTrait.php signingMethods[%s] have unknown status (%s).\n",
            htmlspecialchars($algorithm),
            self::SIGNING_METHODS[$algorithm]
          );
      }
    } else {
  ...
  protected function validateEncryptionMethod($data)
  {
    ...
    if (isset(self::ENCRYPTION_METHODS[$algorithm])) {
      switch (self::ENCRYPTION_METHODS[$algorithm]) {
        ...
        case 'discouraged':
          $this->warning .= sprintf(
            "EncryptionMethod %s is discouraged in xmlenc-core.\n",
            htmlspecialchars($algorithm)
          );
          break;
        case 'obsolete':
        ...
        default:
          $this->result .= sprintf(
            "CommonTrait.php encryptionMethods[%s] have unknown status (%s).\n",
            htmlspecialchars($algorithm),
            self::ENCRYPTION_METHODS[$algorithm]
          );
      }
    } else {
  ...
  protected function checkSAMLEndpoint($data, $type, $saml2, $saml1)
  {
    ...
    switch ($binding) {
      ...
      case 'urn:mace:shibboleth:1.0:profiles:AuthnRequest':
        if (! $saml1) {
          $this->error .= sprintf(
            'urn:mace:shibboleth:1.0 is depending on SAML1.' .
            " Found binding in %s[Binding=%s], but SAML1 not supported in %sDescriptor.\n",
            $name,
            htmlspecialchars($binding),
            $type
          );
      ...
      case 'urn:oasis:names:tc:SAML:2.0:bindings:SOAP-binding':
        $this->error .= sprintf(
          'Binding : %s should be either urn:oasis:names:tc:SAML:2.0:bindings:<b>SOAP</b>' .
          " or urn:oasis:names:tc:SAML:<b>1.0</b>:bindings:SOAP-binding\n",
          htmlspecialchars($binding)
        );
        break;
      case 'urn:oasis:names:tc:SAML:2.0:bindings:artifact-01':
        $this->error .= sprintf(
          'Binding : %s should be either urn:oasis:names:tc:SAML:2.0:bindings:<b>HTTP-Artifact</b>' .
          " or urn:oasis:names:tc:SAML:<b>1.0:profiles</b>:artifact-01\n",
          htmlspecialchars($binding)
        );
        break;
      case 'urn:oasis:names:tc:SAML:2.0:bindings:browser-post':
        $this->error .= sprintf(
          'Binding : %s should be either urn:oasis:names:tc:SAML:2.0:bindings:<b>HTTP-POST</b>' .
          " or urn:oasis:names:tc:SAML:<b>1.0:profiles</b>:browser-post\n",
          htmlspecialchars($binding)
        );
        break;
      default:
   ...
   *
   * Checks if AssertionConsumerService is of thpe Verifies that it's a binding is
   ...
   * Checks NameIDFormat
   *
   * Checks if NameIDFormat is one of the allowed accordording
   *  to https://docs.oasis-open.org/security/saml/v2.0/saml-core-2.0-os.pdf
   *
  ...
  protected function checkNameIDFormat($nameIDFormat)
  {
    ...
    switch ($nameIDFormat) {
      ...
      default:
        $this->warning .= sprintf(
          "Unknown NameIDFormat : %s. See saml-core-2.0-os below 8.3 for options.\n",
